...
def run_simulation():
    ...
    # Grid 
    GRID_SIZE = 100
    grid = [[np.nan for _ in range(GRID_SIZE)] for _ in range(GRID_SIZE)]
    ...
    class Patient:
        ...
        def infect_near(self):
            ...
            for nx, ny in dir:
                near_x = self.posX + nx
                near_y = self.posY + ny
                if 0 <= near_x < GRID_SIZE and 0 <= near_y < GRID_SIZE and not np.isnan(grid[near_x][near_y]):
                    nbr = grid[near_x][near_y]
    ...
    for i in range(MAX_PATIENTS):
        while True:
            ...
            if np.isnan(grid[x][y]):
                patient = Patient(patientNum = i, sick = False, posX = x, posY = y)
    ...
    newlyFile = 'newlyInfMORESPACE.csv'
    totFile = 'totInfMORESPACE.csv'

    new_val_arr = []    # Arr for newlyInfected[Timestep] = new_infected
    tot_inf_arr = []    # Arr for totalInfected[Timestep] = tot_infected

    while(tot_infected < MAX_PATIENTS):     # While the totalInfected < MAX_PATIENTS
